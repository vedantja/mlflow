...
def save_mojo(
    ...
    # Save h2o-model
    if hasattr(h2o, "download_mojo"):
        h2o_save_location = h2o.download_mojo(model=h2o_model, path=model_data_path, get_genmodel_jar=True)
    else:
        warnings.warn(
            "If your cluster is remote, H2O may not store the model correctly. "
            "Please upgrade H2O version to a newer version"
        )
        h2o_save_location = h2o.save_mojo(model=h2o_model, path=model_data_path, force=True)
    model_file = os.path.basename(h2o_save_location)
